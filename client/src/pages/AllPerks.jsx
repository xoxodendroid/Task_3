...
export default function AllPerks() {
  ...
  // Build unique merchants list when perks change
  useEffect(() => {
  ...
  async function loadAllPerks() {
    ...
    setLoading(true)
    try {
      console.log('Loading perks with search:', searchQuery, 'and merchant filter:', merchantFilter)
      ...
      console.log('Perks loaded:', res.data.perks)
  ...
  // Immediate search when the form is submitted (Enter / Search Now)
  ...
  function handleReset() {
    setSearchQuery('')
    setMerchantFilter('')
    // reload all perks (optional immediate refresh)
    loadAllPerks()
  }
  ...
  // Debounced auto-search: runs when searchQuery or merchantFilter change
  useEffect(() => {
    // If you want immediate requests on empty query/filter remove debounce logic
    const controller = { cancelled: false }
    const id = setTimeout(() => {
      if (!controller.cancelled) loadAllPerks()
    }, 400) // 400ms debounce
